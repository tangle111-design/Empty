...
ERROR_CHARS = []
ERROR_ANALYSIS_DATA = [
    {"target_initial": {"error_count": 0}},
    {"target_final": {"error_count": 0}},
]
...
class App(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        ctk.CTkButton(self.btn_frame, text="隐藏提示", command=self.toggle_hint).pack(
            side="left", padx=5
        )
        ctk.CTkButton(
            self.btn_frame,
    ...
    def load_new_char(self):
        ...
        logic = (
            MicrosoftPinyinLogic if self.scheme == "microsoft" else XiaohePinyinLogic
        )
        c1, c2 = (
            logic.get_microsoft_code(self.current_char)
            if self.scheme == "microsoft"
            else logic.get_xiaohe_code(self.current_char)
        )
        ...
        self.code_hint_label.configure(
            text=f"[{self.target_code}]" if self.show_hint else "[??]"
        )
        self.entry.delete(0, "end")

    def check_input(self, event):
        user_input = self.entry.get().lower().strip()
        if not self.start_time and user_input:
    ...
    def switch_scheme(self):
        ...
        self.scheme_btn.configure(
            text=f"当前方案: {'小鹤双拼' if self.scheme=='xiaohe' else '微软双拼'}"
        )
        self.load_new_char()
    ...
    def change_error_practice_mode(self):
        ...
        self.error_practice_btn.configure(
            text="退出错题练习" if self._is_error_practice_mode else "错题练习"
        )
        self.load_new_char()
