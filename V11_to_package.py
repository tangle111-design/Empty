...
# --- 适配打包环境的路径处理 ---
def get_base_path():
    """获取程序运行时的当前目录，确保打包成exe后数据文件存在exe同级目录"""
...
DATA_FILE = os.path.join(BASE_DIR, "practice_data_v2.json")
...
XIAOHE_FINALS_MAP = [
    ...
    {
        "a": "a", "o": "o", "e": "e", "i": "i", "u": "u", "v": "v", "ang": "h",
        "eng": "g", "ia": "x", "ian": "m", "iang": "l", "iao": "n", "ie": "p",
        "in": "b", "ing": "k", "iong": "s", "iu": "q", "ong": "s", "ua": "x",
        "uai": "k", "uan": "r", "uang": "l", "ue": "t", "ui": "v", "un": "y", "uo": "o",
    },
]
...
# --- 补充反映射字典 (用于图表显示) ---
REV_INITIALS = {v: k for k, v in INITIALS_MAP.items()}

REV_MS_FINALS = {}
for py, key in MICROSOFT_FINALS_MAP.items():
    if key not in REV_MS_FINALS: REV_MS_FINALS[key] = []
    REV_MS_FINALS[key].append(py)

REV_XH_FINALS = {}
combined_xh = {**XIAOHE_FINALS_MAP[0], **XIAOHE_FINALS_MAP[1]}
for py, key in combined_xh.items():
    if key not in REV_XH_FINALS: REV_XH_FINALS[key] = []
    REV_XH_FINALS[key].append(py)

# 汉字库 包含所有常见拼音组合
COMMON_CHARS = (
    "八把波伯百白北被班半本奔帮榜崩蹦比必别憋表标边变宾滨兵病不布爬怕婆破拍派陪配"
    "盘判盆噴旁胖朋碰皮批撇瞥票飘片偏品贫平苹普铺妈马摸莫么买卖每美满慢门们忙盲梦"
...
class App(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.progress_label = ctk.CTkLabel(
            self.top_frame, text="", font=("Arial", 20, "bold"), text_color="#F39C12",
        )
        self.progress_label.pack(side="left", padx=150)
        ...
        self.error_analysis_btn = ctk.CTkButton(self.btn_frame, text="错题分析", command=self.show_error_analysis)
        self.error_analysis_btn.pack(side="left", padx=10)

        self.scheme_btn = ctk.CTkButton(
            self.btn_frame, text="当前方案: 小鹤双拼", command=self.switch_scheme,
            fg_color="#3498DB", hover_color="#2980B9",
        )
        self.scheme_btn.pack(side="left", padx=10)

        self.error_practice_btn = ctk.CTkButton(
            self.btn_frame, text="错题消消乐", command=self.change_error_practice_mode,
            fg_color="#F39C12", hover_color="#D35400",
        )
        self.error_practice_btn.pack(side="left", padx=10)

        self.stats_btn = ctk.CTkButton(self.btn_frame, text="查看统计", command=self.show_stats)
        self.stats_btn.pack(side="left", padx=10)

        self.reset_btn = ctk.CTkButton(
            self.top_frame, text="重置数据", command=self.reset_stats,
    ...
    def check_input(self, event):
        ...
        if len(user_input) == 2:
            ...
            if user_input == self.target_code:
                ...
                if self._is_error_practice_mode:
                    char = self.current_char
                    self.error_mastery_tracker[char] = self.error_mastery_tracker.get(char, 0) + 1
                    
                    if self.error_mastery_tracker[char] >= 2:
                        if char in data["error_chars"]:
                            data["error_chars"].remove(char)
                        
                        eliminated = self.initial_error_count - len(data["error_chars"])
                        self.progress_label.configure(text=f"消消乐进度: {eliminated} / {self.initial_error_count}")

                        target_init = self.target_code[0]
                        target_final = self.target_code[1]
                        
                        if target_init in data["error_analysis_data"][0]:
                            data["error_analysis_data"][0][target_init]["error_count"] = max(0, data["error_analysis_data"][0][target_init]["error_count"] - 1)
                        if target_final in data["error_analysis_data"][1]:
                            data["error_analysis_data"][1][target_final]["error_count"] = max(0, data["error_analysis_data"][1][target_final]["error_count"] - 1)
                        
                        del self.error_mastery_tracker[char]
                        
                        if not data["error_chars"]:
                            self.change_error_practice_mode()
                            self.show_info("恭喜！所有错题已消除！")
                ...
                if self._is_error_practice_mode:
                    self.error_mastery_tracker[self.current_char] = 0
    ...
    def show_stats(self):
        ...
        stats_message = (
            ...
            f"待练习错题: {len(data['error_chars'])} 个"
        )
    ...
    def change_error_practice_mode(self):
        ...
        if not self._is_error_practice_mode:
            ...
            self.error_mastery_tracker = {} 
            self.initial_error_count = len(data["error_chars"]) 
            self.progress_label.configure(text=f"消消乐进度: 0 / {self.initial_error_count}")
        else:
    ...
    def show_error_analysis(self):
        ...
        rev_finals_map = REV_MS_FINALS if self.scheme == "microsoft" else REV_XH_FINALS

        init_sorted_raw = sorted([item for item in initial_errors.items() if item[1]['error_count'] > 0], 
                                 key=lambda x: x[1]['error_count'], reverse=True)[:10]
        init_labels = []
        init_values = []
        for key, info in init_sorted_raw:
            init_labels.append(REV_INITIALS.get(key, key))
            init_values.append(info['error_count'])

        final_sorted_raw = sorted([item for item in final_errors.items() if item[1]['error_count'] > 0], 
                                  key=lambda x: x[1]['error_count'], reverse=True)[:10]
        final_labels = []
        final_values = []
        for key, info in final_sorted_raw:
            pys = rev_finals_map.get(key, [key])
            final_labels.append("/".join(pys))
            final_values.append(info['error_count'])

        if not init_values and not final_values:
            self.show_info("暂无有效错误统计（错题已基本消除）")
            return

        analysis_window = ctk.CTkToplevel(self)
        analysis_window.title(f"错题统计分析 - {self.scheme_btn.cget('text')}")
        ...
        ctk.CTkButton(analysis_window, text="关闭界面", font=("Microsoft YaHei", 16), 
                      width=120, height=40, command=analysis_window.destroy).pack(pady=20)

if __name__ == "__main__":
    multiprocessing.freeze_support() # 打包防崩溃必备
    app = App()
    app.mainloop()